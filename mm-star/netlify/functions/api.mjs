...
async function roleAuth(event,roles){
  ...
  try { const {payload}=await jwtVerify(token,jwtSecret); if(!roles.includes(payload.role)) throw new Error('FORBIDDEN'); return payload; }
...
function deliveryLabel(code,l,zone){
  const x={
...
function orderCreatedSms(o){
  ...
  if(l==='en') return `MM STAR ${o.invoice}: ${itemsText(o.items,l)}. Payment: ${paymentLabel(o.paymentMethod,l)}. Delivery: ${deliveryLabel(o.deliveryMethod,l,o.deliveryZone)}. Total: ${total}. Status: ${localizedStatus(statusCodeOf(o),l)}.`;
  if(l==='ru') return `MM STAR ${o.invoice}: ${itemsText(o.items,l)}. Оплата: ${paymentLabel(o.paymentMethod,l)}. Доставка: ${deliveryLabel(o.deliveryMethod,l,o.deliveryZone)}. Итого: ${total}. Статус: ${localizedStatus(statusCodeOf(o),l)}.`;
  if(l==='tr') return `MM STAR ${o.invoice}: ${itemsText(o.items,l)}. Ödeme: ${paymentLabel(o.paymentMethod,l)}. Teslimat: ${deliveryLabel(o.deliveryMethod,l,o.deliveryZone)}. Toplam: ${total}. Durum: ${localizedStatus(statusCodeOf(o),l)}.`;
  return `MM STAR ${o.invoice}: ${itemsText(o.items,l)}. გადახდა: ${paymentLabel(o.paymentMethod,l)}. მიწოდება: ${deliveryLabel(o.deliveryMethod,l,o.deliveryZone)}. სულ: ${total}. სტატუსი: ${localizedStatus(statusCodeOf(o),l)}.`;
}
...
async function keepzCreate(order){
  ...
  const site=(env.PUBLIC_SITE_URL||env.URL||'https://mm-star.netlify.app').replace(/\/$/,'');
  const payload={amount:Number(order.total),receiverId:env.KEEPZ_RECEIVER_ID,receiverType:'BRANCH',integratorId:env.KEEPZ_INTEGRATOR_ID,integratorOrderId:order.keepzOrderId,currency:'GEL',language:String(order.language||'ka').toUpperCase(),callbackUri:`${site}/api/keepz-callback`,successRedirectUri:`${site}/checkout.html?payment=success&order=${encodeURIComponent(order.keepzOrderId)}`,failRedirectUri:`${site}/checkout.html?payment=failed&order=${encodeURIComponent(order.keepzOrderId)}`};
...
async function productItems(requestItems,priceField='price'){
  ...
  for(let n=0;n<snaps.length;n++){
    ...
    const price=Number(p[priceField]||0), weight=Number(p.weightKg||1);
...
async function georgianFont(){
  if(!georgianFontPromise) georgianFontPromise=fetch('https://notofonts.github.io/georgian/fonts/NotoSansGeorgian/hinted/ttf/NotoSansGeorgian-Regular.ttf').then(r=>{if(!r.ok)throw new Error('FONT_FETCH_FAILED');return r.arrayBuffer()}).then(b=>Buffer.from(b));
  return georgianFontPromise;
}
async function invoicePdf(o,isDealer){
  const font=await georgianFont(); const doc=new PDFDocument({margin:42,size:'A4'}), chunks=[];doc.on('data',c=>chunks.push(c));const done=new Promise(r=>doc.on('end',r));doc.registerFont('Geo',font);doc.font('Geo');
  const site=(env.PUBLIC_SITE_URL||env.URL||'https://mm-star.netlify.app').replace(/\/$/,'');
  try{const lr=await fetch(`${site}/assets/logo.jpg`);if(lr.ok)doc.image(Buffer.from(await lr.arrayBuffer()),42,35,{fit:[120,60]})}catch{}
  doc.fontSize(20).text('MM STAR',180,45,{align:'right'});doc.fontSize(12).text(isDealer?'დილერის ინვოისი / DEALER INVOICE':'ინვოისი / INVOICE',180,75,{align:'right'});doc.moveDown(3);
  doc.fontSize(10).text(`ინვოისი: ${o.invoice}`).text(`თარიღი: ${o.createdAt?new Date((o.createdAt._seconds||Date.now()/1000)*1000).toLocaleDateString('ka-GE'):new Date().toLocaleDateString('ka-GE')}`).text(`სტატუსი: ${localizedStatus(statusCodeOf(o),'ka')}`).moveDown();
  const party=isDealer?o.dealer:o.customer;
  if(party){doc.text(`${isDealer?'დილერი':'მომხმარებელი'}: ${party.name||`${party.firstName||''} ${party.lastName||''}`}`);if(party.taxId)doc.text(`საიდენტიფიკაციო კოდი: ${party.taxId}`);if(party.phone)doc.text(`ტელეფონი: ${party.phone}`);if(party.address)doc.text(`მისამართი: ${party.address}`);doc.moveDown();}
  const y=doc.y;doc.fontSize(9).text('კოდი',42,y,{width:80}).text('პროდუქტი',122,y,{width:230}).text('რაოდ.',355,y,{width:45}).text('ფასი',405,y,{width:65,align:'right'}).text('ჯამი',475,y,{width:75,align:'right'});doc.moveTo(42,y+17).lineTo(550,y+17).stroke();doc.y=y+23;
  for(const i of o.items||[]){if(doc.y>720)doc.addPage().font('Geo');const yy=doc.y;doc.text(String(i.code||''),42,yy,{width:80}).text(String(i.name||''),122,yy,{width:225}).text(String(i.qty||0),355,yy,{width:45}).text(`${Number(i.price||0).toFixed(2)} ₾`,405,yy,{width:65,align:'right'}).text(`${Number(i.total||0).toFixed(2)} ₾`,475,yy,{width:75,align:'right'});doc.y=Math.max(doc.y,yy+28);}
  doc.moveDown().fontSize(10).text(`პროდუქტები: ${Number(o.subtotal??o.total??0).toFixed(2)} ₾`,{align:'right'});if(Number(o.deliveryFee||0)>0)doc.text(`მიწოდება: ${Number(o.deliveryFee).toFixed(2)} ₾`,{align:'right'});doc.fontSize(13).text(`სულ: ${Number(o.total||0).toFixed(2)} ₾`,{align:'right'});doc.moveDown();doc.fontSize(9).fillColor('#555').text(`MM STAR • ${DEFAULT_STORE.address} • ${DEFAULT_STORE.onlineSalesPhone} • ${DEFAULT_STORE.email}`,42,760,{align:'center',width:508});doc.end();await done;return Buffer.concat(chunks);
}
...
async function handler(event){
  ...
  try{
    ...
    if(path==='catalog'&&method==='GET'){
      ...
      return ok({products:p.docs.map(publicDoc),categories:c.docs.map(publicDoc),settings:{...DEFAULT_STORE,...(s.exists?s.data():{})},deliveryTariffs:DELIVERY_TARIFFS});
    }
    ...
    if(path==='orders'&&method==='POST'){
      ...
      const lang=language(b.language||profile.preferredLanguage),fee=deliveryFee(pi.weightKg,b.deliveryMethod,b.deliveryZone),total=pi.subtotal+fee,invoice=await nextInvoice('customer'),orderId=id(),ref=db.collection('orders').doc(orderId);
      const order={invoice,customerId:u.uid,customer:{firstName:profile.firstName||'',lastName:profile.lastName||'',phone:u.phone_number||profile.phone||'',email:profile.email||'',address:b.address||profile.address||''},items:pi.items,subtotal:pi.subtotal,weightKg:pi.weightKg,deliveryFee:fee,total,paymentMethod:b.paymentMethod||'invoice',paymentStatus:'UNPAID',deliveryMethod:b.deliveryMethod||'pickup',deliveryZone:b.deliveryZone||'',district:b.district||'',address:b.address||profile.address||'',language:lang,statusCode:'NEW',status:STATUS_KA.NEW,createdAt:admin.firestore.FieldValue.serverTimestamp(),keepzOrderId:orderId};
      await db.runTransaction(async tx=>{await decrementStock(tx,pi.refs,pi.items);tx.set(ref,order)});
      let payment=null;if(order.paymentMethod==='keepz'){try{payment=await keepzCreate({...order,keepzOrderId:orderId});await ref.update({keepz:payment,paymentStatus:'PENDING'});}catch(e){await restoreStock(order.items);await ref.delete();throw e;}}
      await safeSms(order.customer.phone,orderCreatedSms(order));return ok({orderId,invoice,total,deliveryFee:fee,payment});
    }
    if(path==='keepz-callback'&&method==='POST'){
      let payload=bodyOf(event);try{payload=keepzDecrypt(payload)}catch{}const oid=payload.integratorOrderId;if(oid){const ref=db.collection('orders').doc(oid),s=await ref.get();if(s.exists){const success=['SUCCESS','PAID','COMPLETED'].includes(String(payload.status||'').toUpperCase()),o=s.data(),code=success?'PAID':statusCodeOf(o);await ref.update({paymentStatus:success?'PAID':'FAILED',paymentCallback:payload,statusCode:code,status:STATUS_KA[code]||o.status,updatedAt:admin.firestore.FieldValue.serverTimestamp()});if(success&&statusCodeOf(o)!=='PAID')await safeSms(o.customer?.phone,statusSms(o,'PAID'));}}return json(200,{received:true});
    }
    ...
    if(path==='admin/dashboard'&&method==='GET'){await roleAuth(event,['admin']);const[p,c,u,d,o,doq]=await Promise.all(['products','categories','customers','dealers','orders','dealerOrders'].map(x=>db.collection(x).get()));const sales=o.docs.reduce((a,s)=>a+(s.data().paymentStatus==='PAID'?Number(s.data().total||0):0),0);return ok({stats:{products:p.size,categories:c.docs.filter(x=>!x.data().parentId).length,subcategories:c.docs.filter(x=>x.data().parentId).length,customers:u.size,dealers:d.size,orders:o.size,dealerOrders:doq.size,sales}})}
    if(path==='admin/products'&&method==='GET'){await roleAuth(event,['admin']);const q=await db.collection('products').get();return ok({products:q.docs.map(publicDoc).sort((a,b)=>String(a.name).localeCompare(String(b.name)))})}
    if(path==='admin/products'&&method==='POST'){await roleAuth(event,['admin']);const b=bodyOf(event),ref=b.id?db.collection('products').doc(b.id):db.collection('products').doc();await ref.set({code:b.code||'',name:b.name||'',description:b.description||'',categoryId:b.categoryId||'',subcategoryId:b.subcategoryId||'',image:b.image||'',price:Number(b.price||0),dealerPrice:Number(b.dealerPrice||0),stock:Number(b.stock||0),weightKg:Math.max(.01,Number(b.weightKg||1)),active:b.active!==false,isNew:!!b.isNew,updatedAt:admin.firestore.FieldValue.serverTimestamp(),...(!b.id?{createdAt:admin.firestore.FieldValue.serverTimestamp()}: {})},{merge:true});return ok({id:ref.id})}
    ...
    if(path==='admin/order-status'&&method==='POST'){await roleAuth(event,['admin']);const b=bodyOf(event),coll=b.dealer?'dealerOrders':'orders',ref=db.collection(coll).doc(b.id),s=await ref.get();if(!s.exists)return bad('NOT_FOUND',404);const code=STATUS_CODES.includes(b.statusCode)?b.statusCode:(STATUS_FROM_OLD[b.status]||b.status||'NEW');await ref.update({statusCode:code,status:STATUS_KA[code]||code,updatedAt:admin.firestore.FieldValue.serverTimestamp()});const o=s.data(),phone=b.dealer?o.dealer?.phone:o.customer?.phone;if(code!==statusCodeOf(o))await safeSms(phone,statusSms(o,code));return ok()}
    ...
    if(path==='admin/dealer-orders'&&method==='GET'){await roleAuth(event,['admin']);const q=await db.collection('dealerOrders').limit(300).get();return ok({orders:q.docs.map(publicDoc).sort((a,b)=>(b.createdAt?._seconds||0)-(a.createdAt?._seconds||0))})}
    ...
    if(path==='dealer/catalog'&&method==='GET'){const a=await roleAuth(event,['dealer']),ds=await db.collection('dealers').doc(a.sub).get(),d=ds.data(),q=await db.collection('products').where('active','==',true).get(),cats=await db.collection('categories').get();const products=q.docs.map(publicDoc).filter(p=>(d.categories||[]).length===0||(d.categories||[]).includes(p.categoryId)||(d.categories||[]).includes(p.subcategoryId));return ok({products:products.map(p=>({...p,retailPrice:p.price,price:p.dealerPrice,dealerPrice:p.dealerPrice,difference:Number(p.price)-Number(p.dealerPrice)})),categories:cats.docs.map(publicDoc),dealer:{name:d.name,preferredLanguage:d.preferredLanguage||'ka'}})}
