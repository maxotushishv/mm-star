...
async function invoicePdf(o,isDealer){
  const font=await georgianFont(),settings=await storeSettings();const doc=new PDFDocument({margin:42,size:'A4'}),chunks=[];doc.on('data',c=>chunks.push(c));const done=new Promise(r=>doc.on('end',r));doc.registerFont('Geo',font);
  const site='https://mmstar.ge';
  try{const lr=await fetch(`${site}/assets/logo.jpg`);if(lr.ok)doc.image(Buffer.from(await lr.arrayBuffer()),42,35,{fit:[120,60]})}catch{}
  doc.font('Helvetica-Bold').fontSize(20).text('MM STAR',180,45,{align:'right'});
  doc.font('Geo').fontSize(12).text(isDealer?'დილერის ინვოისი':'ინვოისი',180,75,{align:'right'});doc.font('Helvetica').fontSize(9).text(isDealer?'DEALER INVOICE':'INVOICE',180,92,{align:'right'});doc.y=125;
  const label=(ka,val)=>{const y=doc.y;doc.font('Geo').fontSize(10).text(ka,42,y,{continued:false,width:155});doc.font('Helvetica').fontSize(10).text(String(val??''),197,y,{width:353});doc.y=Math.max(doc.y,y+18)};
  label('ინვოისი',o.invoice||'');label('თარიღი',o.createdAt?new Date((o.createdAt._seconds||Date.now()/1000)*1000).toLocaleDateString('ka-GE'):new Date().toLocaleDateString('ka-GE'));
  doc.font('Geo').fontSize(10).text(`სტატუსი: ${localizedStatus(statusCodeOf(o),'ka')}`,42,doc.y);doc.moveDown(.7);
  const party=isDealer?o.dealer:o.customer;if(party){doc.font('Geo').text(`${isDealer?'დილერი':'მომხმარებელი'}: ${party.name||`${party.firstName||''} ${party.lastName||''}`}`);if(party.taxId)label('საიდენტიფიკაციო კოდი',party.taxId);if(party.phone)label('ტელეფონი',party.phone);if(party.address)doc.font('Geo').text(`მისამართი: ${party.address}`);doc.moveDown(.7)}
  const y=doc.y;doc.font('Geo').fontSize(9).text('კოდი',42,y,{width:80}).text('პროდუქტი',122,y,{width:225}).text('რაოდ.',355,y,{width:45}).text('ფასი',405,y,{width:65,align:'right'}).text('ჯამი',475,y,{width:75,align:'right'});doc.moveTo(42,y+17).lineTo(550,y+17).stroke();doc.y=y+23;
  for(const i of o.items||[]){if(doc.y>720){doc.addPage();doc.y=50}const yy=doc.y;doc.font('Helvetica').fontSize(9).text(String(i.code||''),42,yy,{width:80});doc.font('Geo').text(String(i.name||''),122,yy,{width:225});doc.font('Helvetica').text(String(i.qty||0),355,yy,{width:45}).text(`${Number(i.price||0).toFixed(2)} GEL`,405,yy,{width:65,align:'right'}).text(`${Number(i.total||0).toFixed(2)} GEL`,475,yy,{width:75,align:'right'});doc.y=yy+28}
  const moneyLine=(ka,n,size=10)=>{const y=doc.y;doc.font('Geo').fontSize(size).text(ka,355,y,{width:90,align:'right'});doc.font(size>=13?'Helvetica-Bold':'Helvetica').fontSize(size).text(`${Number(n||0).toFixed(2)} GEL`,448,y,{width:102,align:'right'});doc.y=y+20};doc.moveDown(.5);moneyLine('პროდუქტები',o.subtotal??o.total,10);if(Number(o.deliveryFee||0)>0)moneyLine('მიწოდება',o.deliveryFee,10);moneyLine('სულ',o.total,13);
  doc.font('Geo').fontSize(8).fillColor('#555').text(`${settings.address||DEFAULT_STORE.address}`,42,748,{align:'center',width:508});doc.font('Helvetica').fontSize(8).text(`MM STAR | ${settings.onlineSalesPhone||DEFAULT_STORE.onlineSalesPhone} | ${settings.email||DEFAULT_STORE.email} | mmstar.ge`,42,765,{align:'center',width:508});doc.end();await done;return Buffer.concat(chunks)
}
