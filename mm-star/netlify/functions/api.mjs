...
async function handler(event){
  ...
  try {
    ...
    if(path==='orders'&&method==='GET'){const u=await firebaseAuth(event), q=await db.collection('orders').where('customerId','==',u.uid).limit(100).get(); const orders=q.docs.map(productPublic).sort((a,b)=>(b.createdAt?._seconds||0)-(a.createdAt?._seconds||0)); return ok({orders});}
    if(path==='orders'&&method==='POST'){
      ...
      await db.runTransaction(async tx=>{const fresh=[];for(const r of refs)fresh.push(await tx.get(r));for(let n=0;n<fresh.length;n++){const p=fresh[n].data(),qty=items[n].qty;if(Number(p.stock)<qty)throw new Error('OUT_OF_STOCK');}for(let n=0;n<refs.length;n++)tx.update(refs[n],{stock:admin.firestore.FieldValue.increment(-items[n].qty),sales:admin.firestore.FieldValue.increment(items[n].qty)});tx.set(ref,{invoice,customerId:u.uid,customer:{firstName:profile.firstName||'',lastName:profile.lastName||'',phone:u.phone_number||profile.phone||'',email:profile.email||'',address:b.address||profile.address||''},items,subtotal:total,deliveryFee:0,total,paymentMethod:b.paymentMethod||'invoice',paymentStatus:'UNPAID',deliveryMethod:b.deliveryMethod||'pickup',status:'ახალი',createdAt:admin.firestore.FieldValue.serverTimestamp(),keepzOrderId:orderId});});
